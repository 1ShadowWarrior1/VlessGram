...
import org.telegram.vless.VlessRepository;
import org.telegram.vless.VlessRepository.SubscriptionInfo;
import org.telegram.vless.VlessRepository.VlessNode;
...
public class ProxyListActivity extends BaseFragment implements NotificationCenter.NotificationCenterDelegate {
    ...
    // --- VLESS injection block (manual keys + subscription nodes) ---
    private VlessRepository vlessRepository;
    private int vlessStartRow = -1;
    private int vlessEndRow = -1;
    private List<VlessRowItem> vlessRows = Collections.emptyList();
    private final VlessRepository.Listener vlessListener = this::onVlessDataChanged;

    /** Row item for VLESS section, matching example layout. */
    private static final class VlessRowItem {
        static final int TYPE_DIVIDER = 0;
        static final int TYPE_ACTION = 1;
        static final int TYPE_HEADER = 2;
        static final int TYPE_NODE = 5;
        static final int TYPE_SWITCH = 6;
        ...
        final Runnable infoClick;
        final String subtext;

        VlessRowItem(int type, String title, String actionType, String subId, VlessNode node, Runnable infoClick, String subtext) {
            this.type = type;
            this.title = title;
        ...
        static VlessRowItem divider() {
            return new VlessRowItem(TYPE_DIVIDER, null, null, null, null, null, null);
        }

        static VlessRowItem action(String title, String actionType, String subId) {
            return new VlessRowItem(TYPE_ACTION, title, actionType, subId, null, null, null);
        }

        static VlessRowItem header(String title) {
            return new VlessRowItem(TYPE_HEADER, title, null, null, null, null, null);
        }

        static VlessRowItem node(VlessNode node, Runnable infoClick) {
            return new VlessRowItem(TYPE_NODE, null, null, null, node, infoClick, null);
        }

        static VlessRowItem switchRow(String title, String actionType, String subtext) {
            return new VlessRowItem(TYPE_SWITCH, title, actionType, null, null, null, subtext);
        }
    }
        ...
        public void updateStatus() {
            ...
            if (SharedConfig.currentProxy == currentInfo && useProxySettings) {
                ...
                if (currentInfo.checking) {
                    ...
                    // Ping not checked yet
                    valueTextView.setText("Не проверено");
                    colorKey = Theme.key_windowBackgroundWhiteGrayText2;
    ...
    public boolean onFragmentCreate() {
        ...
        vlessRepository = VlessRepository.getInstance();
        vlessRepository.addListener(vlessListener);
        // Don't call ensureAutoConnect here - it's already called on app startup
        // Calling it here causes VPN to disconnect when entering proxy menu
    ...
    public void onFragmentDestroy() {
        ...
        if (vlessRepository != null) {
            vlessRepository.removeListener(vlessListener);
        }
    ...
    private void onVlessDataChanged() {
        updateRows(true);
    }

    private List<VlessRowItem> rebuildVlessRows() {
        if (vlessRepository == null) return Collections.emptyList();

        List<VlessRowItem> rows = new ArrayList<>();
        boolean sortEnabled = vlessRepository.isSortByPing();
        List<VlessNode> manualNodes = vlessRepository.getManualNodes();
        List<SubscriptionInfo> subs = vlessRepository.getSubscriptions();

        int subNodesTotal = 0;
        for (SubscriptionInfo s : subs) subNodesTotal += s.nodes.size();
        // Even if there are no nodes yet, keep the section visible
        // so the user can add the first key/subscription.

        // Заголовок секции VPN
        rows.add(VlessRowItem.divider());
        rows.add(VlessRowItem.header("НАСТРОЙКИ VPN")); // Капсом для акцента

        // Включаем добавление ключей/подписок прямо из списка прокси,
        // чтобы убрать соответствующий UI из ProxySettingsActivity.
        rows.add(VlessRowItem.action("➕ Добавить ключ", "add_key", null));
        rows.add(VlessRowItem.action("➕ Добавить подписку", "add_sub", null));

        // Переключатели
        rows.add(VlessRowItem.switchRow("⚡ Автовключение", "auto_enable_vpn", "Подключать VPN автоматически при запуске"));
        rows.add(VlessRowItem.switchRow("🔄 Умное переключение", "auto_switch_vpn", "Искать новый узел, если текущий недоступен"));
        
        // Кнопки действий
        // Для сортировки передаем статус в третьем параметре (actionType или subId), чтобы отобразить его справа
        rows.add(VlessRowItem.action("📶 Сортировка по пингу", "toggle_sort", sortEnabled ? "Вкл." : "Выкл."));
        rows.add(VlessRowItem.action("🔎 Проверить задержку всех узлов", "check_ping", null));

        Comparator<VlessNode> nodeComparator = Comparator
                .comparingLong((VlessNode n) -> n.proxyInfo.ping >= 0 ? n.proxyInfo.ping : 9999)
                .thenComparing(n -> n.name == null ? "" : n.name);

        // Секция личных ключей
        if (!manualNodes.isEmpty()) {
            rows.add(VlessRowItem.divider());
            rows.add(VlessRowItem.header("МОИ КЛЮЧИ"));
            List<VlessNode> sortedManual = new ArrayList<>(manualNodes);
            if (sortEnabled) sortedManual.sort(nodeComparator);
            for (VlessNode node : sortedManual) {
                rows.add(VlessRowItem.node(node, () -> showDeleteKeyDialog(node.uri)));
            }
        }

        // Секция подписок
        for (SubscriptionInfo sub : subs) {
            if (sub.nodes.isEmpty()) continue;
            rows.add(VlessRowItem.divider());
            rows.add(VlessRowItem.header(sub.name.toUpperCase()));
            
            List<VlessNode> subNodes = new ArrayList<>(sub.nodes);
            if (sortEnabled) subNodes.sort(nodeComparator);
            for (VlessNode node : subNodes) {
                rows.add(VlessRowItem.node(node, null));
            }
            rows.add(VlessRowItem.action("♻️ Обновить подписку", "update_sub", sub.id));
            rows.add(VlessRowItem.action("🗑 Удалить подписку", "delete_sub", sub.id));
        }
    ...
    private void showDeleteKeyDialog(String uri) {
        AlertDialog.Builder builder = new AlertDialog.Builder(getParentActivity());
        builder.setMessage(LocaleController.getString(R.string.DeleteProxyConfirm));
        ...
        builder.setPositiveButton(LocaleController.getString(R.string.Delete), (d, w) -> {
            if (vlessRepository != null) vlessRepository.deleteManualKeyAsync(uri, null);
        });
    ...
    private void showDeleteSubscriptionDialog(String subId) {
        AlertDialog.Builder builder = new AlertDialog.Builder(getParentActivity());
        builder.setMessage("Удалить подписку?");
        builder.setTitle(LocaleController.getString(R.string.DeleteProxyTitle));
        builder.setPositiveButton(LocaleController.getString(R.string.Delete), (d, w) -> {
            if (vlessRepository != null) vlessRepository.deleteSubscriptionAsync(subId, null);
        });
        builder.setNegativeButton(LocaleController.getString(R.string.Cancel), null);
        AlertDialog dialog = builder.create();
        showDialog(dialog);
    ...
    private void showAddKeyDialog() {
        ...
        builder.setPositiveButton(LocaleController.getString(R.string.Add), (d, w) -> {
            if (vlessRepository == null) return;
            String text = input.getText() != null ? input.getText().toString().trim() : "";
            if (text.length() > 5) {
                vlessRepository.addManualKeyAsync(text, null);
            }
        });
        builder.setNegativeButton(LocaleController.getString(R.string.Cancel), null);
    ...
    private void showAddSubscriptionDialog() {
        AlertDialog.Builder builder = new AlertDialog.Builder(getParentActivity());
        builder.setTitle("Добавить подписку (URL)");
        ...
        builder.setPositiveButton(LocaleController.getString(R.string.Add), (d, w) -> {
            ...
            String text = input.getText() != null ? input.getText().toString().trim() : "";
            if (text.length() > 5) {
                vlessRepository.addOrUpdateSubscriptionAsync(text, null);
            }
        });
        builder.setNegativeButton(LocaleController.getString(R.string.Cancel), null);
    ...
    public View createView(Context context) {
        ...
        listView = new RecyclerListView(context) {
            ...
            protected void dispatchDraw(Canvas canvas) {
                ...
                if (vlessStartRow >= 0 && vlessEndRow > vlessStartRow) {
                    drawSectionBackground(canvas, vlessStartRow, vlessEndRow - 1, Theme.getColor(Theme.key_windowBackgroundWhite));
                }
        ...
        listView.setOnItemClickListener((view, position) -> {
            if (position == useProxyRow) {
                ...
                VlessRowItem item = vlessRows.get(position - vlessStartRow);
                if (item.type == VlessRowItem.TYPE_ACTION) {
                    if ("check_ping".equals(item.actionType) && vlessRepository != null) {
                        vlessRepository.checkPingAllAsync();
                    } else if ("toggle_sort".equals(item.actionType) && vlessRepository != null) {
                        vlessRepository.toggleSortByPing();
                    } else if ("update_sub".equals(item.actionType) && item.subId != null && vlessRepository != null) {
                        vlessRepository.updateSubscriptionAsync(item.subId, null);
                    } else if ("delete_sub".equals(item.actionType) && item.subId != null) {
                        showDeleteSubscriptionDialog(item.subId);
                    } else if ("add_key".equals(item.actionType) && vlessRepository != null) {
                        showAddKeyDialog();
                    } else if ("add_sub".equals(item.actionType) && vlessRepository != null) {
                        showAddSubscriptionDialog();
                    }
                } else if (item.type == VlessRowItem.TYPE_SWITCH && vlessRepository != null) {
                    if ("auto_enable_vpn".equals(item.actionType)) {
                        vlessRepository.setAutoEnableVpn(!vlessRepository.getAutoEnableVpn());
                        listAdapter.notifyItemChanged(position);
                    } else if ("auto_switch_vpn".equals(item.actionType)) {
                        vlessRepository.setAutoSwitchVpn(!vlessRepository.getAutoSwitchVpn());
                        listAdapter.notifyItemChanged(position);
                    }
                } else if (item.type == VlessRowItem.TYPE_NODE && item.node != null && vlessRepository != null) {
                    vlessRepository.toggleConnectAsync(item.node.uri);
                }
        ...
        listView.setOnItemLongClickListener((view, position) -> {
            if (position >= vlessStartRow && position < vlessEndRow) {
                VlessRowItem item = vlessRows.get(position - vlessStartRow);
                if (item.type == VlessRowItem.TYPE_NODE && item.node != null) {
                    if (item.node.manual) {
                        showDeleteKeyDialog(item.node.uri);
                    } else if (item.node.subscriptionId != null) {
                        showDeleteSubscriptionDialog(item.node.subscriptionId);
                    }
                    return true;
                }
            }
    ...
    private void updateRows(boolean notify) {
        ...
        // VLESS section — full menu layout matching example.
        vlessRows = rebuildVlessRows();
        if (!vlessRows.isEmpty()) {
            vlessStartRow = rowCount;
            rowCount += vlessRows.size();
    ...
    /**
     * Removes outdated local proxies from SharedConfig list so it does not accumulate
     * stale 127.0.0.1 entries. Keeps only current local proxy (if any).
     */
    private void cleanupStaleLocalProxies() {
        SharedConfig.ProxyInfo current = SharedConfig.currentProxy;
        boolean currentIsLocal = current != null && LOCAL_PROXY_HOST.equals(current.address);
        ArrayList<SharedConfig.ProxyInfo> toDelete = new ArrayList<>();
        for (SharedConfig.ProxyInfo info : SharedConfig.proxyList) {
            if (info == null || !LOCAL_PROXY_HOST.equals(info.address)) {
                continue;
            }
            if (currentIsLocal && info == current) {
                continue;
            }
            toDelete.add(info);
        }
    ...
    public void didReceivedNotification(int id, int account, Object... args) {
        if (id == NotificationCenter.proxyChangedByRotation) {
            ...
            // Re-read proxy state from prefs — VlessRepository may have changed it on a bg thread.
            SharedPreferences proxyPrefs = MessagesController.getGlobalMainSettings();
            useProxySettings = proxyPrefs.getBoolean("proxy_enabled", false) && !SharedConfig.proxyList.isEmpty();
            useProxyForCalls = proxyPrefs.getBoolean("proxy_enabled_calls", false);
            updateRows(true);
            if (vlessRepository != null) {
                boolean enabled = proxyPrefs.getBoolean("proxy_enabled", false);
                String active = vlessRepository.getActiveUri();
                if (!enabled && active != null && !active.isEmpty()) {
                    vlessRepository.disconnectAsync();
                }
            }
    ...
    private class ListAdapter extends RecyclerListView.SelectionAdapter {
            ...
            switch (holder.getItemViewType()) {
                ...
                case VIEW_TYPE_TEXT_SETTING: {
                    ...
                    if (position == proxyAddRow) {
                        ...
                        if (item.type == VlessRowItem.TYPE_ACTION) {
                            // Если это кнопка удаления - красим в красный
                            if ("delete_sub".equals(item.actionType)) {
                                textCell.setTextColor(Theme.getColor(Theme.key_text_RedRegular));
                                textCell.setText(item.title, false);
                            } 
                            // Если это сортировка - показываем статус справа
                            else if ("toggle_sort".equals(item.actionType)) {
                                textCell.setTextColor(Theme.getColor(Theme.key_windowBackgroundWhiteBlackText));
                                // item.subId здесь содержит наш текст "Вкл." или "Выкл."
                                textCell.setTextAndValue(item.title, item.subId, false);
                            }
                            // Для всех остальных действий (Пинг, Обновить) - синий "активный" цвет
                            else {
                                textCell.setTextColor(Theme.getColor(Theme.key_windowBackgroundWhiteBlueText4));
                                textCell.setText(item.title, false);
                            }
                ...
                case VIEW_TYPE_HEADER: {
                    ...
                    if (position == connectionsHeaderRow) {
                        ...
                        VlessRowItem item = vlessRows.get(position - vlessStartRow);
                        if (item.type == VlessRowItem.TYPE_HEADER) {
                ...
                case VIEW_TYPE_TEXT_CHECK: {
                    ...
                    if (position == useProxyRow) {
                        ...
                        VlessRowItem item = vlessRows.get(position - vlessStartRow);
                        if (item.type == VlessRowItem.TYPE_SWITCH && vlessRepository != null) {
                            boolean checked = "auto_enable_vpn".equals(item.actionType) ? vlessRepository.getAutoEnableVpn() : vlessRepository.getAutoSwitchVpn();
                            checkCell.setTextAndValueAndCheck(item.title, item.subtext != null ? item.subtext : "", checked, item.subtext != null, false);
                        }
                    }
                    break;
                }
                ...
                case VIEW_TYPE_PROXY_DETAIL: {
                    ...
                    if (position >= proxyStartRow && position < proxyEndRow) {
                        ...
                        cell.setInfoClickListener(null);
                    } else if (position >= vlessStartRow && position < vlessEndRow) {
                        VlessRowItem item = vlessRows.get(position - vlessStartRow);
                        if (item.type == VlessRowItem.TYPE_NODE && item.node != null) {
                            VlessNode node = item.node;
                            cell.setProxy(node.proxyInfo);
                            cell.setTitle(node.name != null ? node.name : "");
                            // Use activeUri so the checkmark is shown immediately on startup,
                            // even before connectInternal finishes and sets SharedConfig.currentProxy.
                            String activeUri = vlessRepository != null ? vlessRepository.getActiveUri() : "";
                            cell.setChecked(node.uri.equals(activeUri));
                            cell.setSelectionEnabled(false, false);
                            cell.setItemSelected(false, false);
                            cell.setInfoClickListener(item.infoClick != null ? v -> item.infoClick.run() : null);
                            cell.updateStatus();
                        }
                    }
                    break;
        ...
        public boolean isEnabled(RecyclerView.ViewHolder holder) {
            ...
            if (position >= vlessStartRow && position < vlessEndRow) {
                VlessRowItem item = vlessRows.get(position - vlessStartRow);
                return item.type != VlessRowItem.TYPE_DIVIDER;
            }
        ...
        public long getItemId(int position) {
            ...
            if (position == useProxyShadowRow) {
                ...
                VlessRowItem item = vlessRows.get(position - vlessStartRow);
                if (item.type == VlessRowItem.TYPE_NODE && item.node != null) return item.node.uri.hashCode();
                return (long) (position + 10000);
            } else {
                return -7;
            }
        }
        ...
        public int getItemViewType(int position) {
            if (position == useProxyShadowRow || position == proxyShadowRow) {
                ...
                VlessRowItem item = vlessRows.get(position - vlessStartRow);
                if (item.type == VlessRowItem.TYPE_DIVIDER) return VIEW_TYPE_SHADOW;
                if (item.type == VlessRowItem.TYPE_ACTION) return VIEW_TYPE_TEXT_SETTING;
                if (item.type == VlessRowItem.TYPE_HEADER) return VIEW_TYPE_HEADER;
                if (item.type == VlessRowItem.TYPE_NODE) return VIEW_TYPE_PROXY_DETAIL;
                if (item.type == VlessRowItem.TYPE_SWITCH) return VIEW_TYPE_TEXT_CHECK;
                return VIEW_TYPE_SHADOW;
